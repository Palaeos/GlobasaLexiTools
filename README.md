...
- Add
  - example sentences with English translations
    - gather such examples in a Python dictionary which takes priority over the corpus examples
  - sounds
- complete images
  - add routine to copy over image links to extended Menalari
  - fetch images from Wikimedia commons and manually vet them
## Wiktionary
...
- [`Word selection automator.py`](Word%20selection%20automator.py) - asks for an English word, then part of speech and finally the intended sense to scrape a Wiktionary dump to automatically generate a lexiseleti complete with Globasa renditions of the phonetics. **Requires**: [`cutlet`](https://github.com/polm/cutlet) for Japanese romanization, [`korean_romanizer`](https://github.com/osori/korean-romanizer) for Korean romanization
  - [`GlobasaTransliterators.py`](GlobasaSourceTransliterators.py) - is used to convert official romanizations and Latin spellings into Globasa renditions of the phonetics

### TODO

Make a scraper that stores the Wiktionary data in the following structure of nested dictionaries:

`{English Word: {Sense: {Language: Translation}}}
`
